...
- `e2e.yml` — **Full-stack E2E** (added 2026-07, RS-32). Needs the whole stack, so it runs a full `mvn package` (frontend baked into `src/main/resources/static`, no `-DskipFrontend`), then lets Playwright boot the resulting jar and run the critical-flow suite in headless Chromium. `frontend-maven-plugin` installs the webapp deps (incl. the Playwright runner) during the build; `npx playwright install --with-deps chromium` fetches the browser. Triggers on `src/main/**`, `pom.xml`, and its own file. Kept separate from the unit workflows so those stay fast; the Playwright HTML report is archived as an artifact.
