...
- `frontend.yml` — **Frontend only**. Runs `npm ci`, `npm run lint`, `npm run build`, and `npm test` (Angular/Karma) on Node 26.8.1. Triggers only on `src/main/webapp/**`. Lint enabled 2026-05-03 after the standalone + `inject()` migration cleared the `prefer-inject` backlog (134 errors → 0).
