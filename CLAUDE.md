...
- **Frontend:** Angular 21 under `src/main/webapp/`, built by `frontend-maven-plugin` 2.0.0 as part of the Maven build (Node 24.15.0 Krypton LTS, npm 11.12.1 downloaded by the plugin). Migrated Angular 15 → 20 → 21 via step-by-step `ng update` chain in 2026-04. RxJS 7.8, TypeScript 5.9, zone.js 0.16. Templates use `@if`/`@for` block control-flow (auto-migrated by `ng update` Angular 20 → 21; no `*ngIf`/`*ngFor` left). **Standalone components throughout** — `app.module.ts` deleted 2026-05-03 via `ng generate @angular/core:standalone` (3-phase schematic: convert-to-standalone → prune-ng-modules → standalone-bootstrap). DI uses `inject()` not constructor params, migrated same day via `ng generate @angular/core:inject`. Bootstrap is `bootstrapApplication(AppComponent, { providers: [...] })` in `main.ts`. Linting: ESLint 9 + `angular-eslint` + `typescript-eslint` (TSLint + codelyzer removed); enforced in CI as of 2026-05-03 (was disabled while 134 `prefer-inject` errors lingered, all cleared by the inject migration). No E2E tests — Protractor + `e2e/` removed (EOL 2023, no replacement wired up yet; Playwright/Cypress would be the natural choice if E2E coverage is ever added).
